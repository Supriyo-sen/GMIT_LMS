...
export async function getPaymentOverView(token) {
  const toastId = toast.loading("Loading Payment Overview...");
  let result = [];
  try {
    const response = await apiConnector("GET", PAYMENT_OVERVIEW_API, null, {
      Authorization: `Bearer ${token}`,
    });
    ...
    console.log("GET_PAYMENT_OVERVIEW_API ERROR....", error);
    toast.error("Could not get Payment Overview");
  }
  toast.dismiss(toastId);
  return result;
}

export async function getPaymentLogs(token) {
  const toastId = toast.loading("Loading Payment Logs...");
  let result = [];
  try {
    const response = await apiConnector("GET", PAYMENT_LOGS_API, null, {
      Authorization: `Bearer ${token}`,
    });
